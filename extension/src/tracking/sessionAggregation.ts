...
export function aggregateSessions(
  ...
  const best = filtered.reduce<SessionHistoryRecord | null>((bestRecord, record) => (
    !bestRecord || record.metrics.shippingActivity + record.metrics.focusScore
      > bestRecord.metrics.shippingActivity + bestRecord.metrics.focusScore ? record : bestRecord
  ), null);
...
export function calculateLongestStreak(records: readonly SessionHistoryRecord[], now = Date.now()): number {
