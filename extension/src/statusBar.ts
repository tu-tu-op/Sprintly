...
export class SprintlyStatusBar implements vscode.Disposable {
  ...
  constructor(private readonly tracker: SessionTracker, context: vscode.ExtensionContext) {
...
export function updateTooltip(
  ...
  md.appendMarkdown('---\n\n');
  md.appendMarkdown(
    `[$(graph) Open Panel](command:sprintly.showStatusPanel)  ` +
...
export function makeProgressBar(pct: number): string {
...
export function startLiveUpdates(
  ...
  const tick = () => {
    try {
      const s = tracker.get();
      if (!s.isRecording) { return; }   // idle state handled by update()
  ...
  context.subscriptions.push(
    tracker.onDidUpdate.event(tick),
  );
}
...
function activityPct(edits: number): number {
