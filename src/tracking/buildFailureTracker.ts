...
import { classifyTerminalCommand } from './terminalCommands';

export const FAILURE_PATTERNS: Array<{ id: string; pattern: RegExp }> = [
  { id: 'build_failure', pattern: /build failed|build failure|compilation failed|compile failed/i },
...
export function categorizeFailure(output: string, commandLine = ''): string {
  const matched = FAILURE_PATTERNS.find(({ pattern }) => pattern.test(output));
  if (matched) return matched.id;
  const commandCategory = classifyTerminalCommand(commandLine);
  if (commandCategory === 'build') return 'build_failure';
  if (commandCategory === 'test') return 'test_failure';
  if (commandCategory === 'lint') return 'lint_failure';
  return 'other';
}
