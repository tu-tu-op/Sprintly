 ...
 */
export function calculateSessionScore(input: DeveloperMetricInput): SessionScore {
  const metrics = calculateDeveloperMetrics(input);
  const consistency = percentage(
    Math.min(Math.max(0, input.fileSaves), Math.max(1, input.fileEdits)),
    Math.max(1, input.fileEdits),
  );
  const aiBalance = balancedAiScore(metrics.aiBalance);
  const devScore = Math.round(
    metrics.focusScore * 0.25
      + consistency * 0.15
...
function balancedAiScore(aiBalance: number): number {
  return aiBalance > 0
