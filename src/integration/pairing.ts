import type { HttpRequest } from './sprintlyApi';
import { nodeHttpRequest } from './sprintlyApi';

export type PairingDeviceType = 'vscode' | 'desktop' | 'other';
...
export interface HttpPairingAdapterOptions {
  baseUrl: string;
  request?: HttpRequest;
  timeoutMs?: number;
}

export class SprintlyPairingError extends Error {
  readonly name = 'SprintlyPairingError';
 ...
 */
export class HttpPairingAdapter implements SprintlyPairingAdapter {
  private readonly baseUrl: URL;
  private readonly request: HttpRequest;
  private readonly timeoutMs: number;

  constructor(options: HttpPairingAdapterOptions) {
    this.baseUrl = parseBaseUrl(options.baseUrl);
    this.request = options.request ?? nodeHttpRequest;
    this.timeoutMs = options.timeoutMs ?? 15_000;
  }

  async exchangeCode(request: PairingExchangeRequest): Promise<PairingExchangeResponse> {
    ...
    if (response.status < 200 || response.status >= 300) {
      throw new SprintlyPairingError(
        pairingFailureMessage(response.status),
