...
export class SessionTracker implements vscode.Disposable {
  ...
  stop(stoppedAt = Date.now()): void {
    if (this.stats.startedAt) {
      const pausedSeconds = this.stats.isPaused && this.stats.pausedAt
        ? Math.max(0, (stoppedAt - this.stats.pausedAt.getTime()) / 1000)
        : 0;
      this.stats.totalPausedSeconds += pausedSeconds;
      this.stats.durationSeconds = Math.max(0, Math.floor(
        (stoppedAt - this.stats.startedAt.getTime()) / 1000 - this.stats.totalPausedSeconds,
      ));
    }
