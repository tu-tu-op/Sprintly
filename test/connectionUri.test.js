...
test('automatic pairing URI rejects malformed links and never accepts a token field', () => {
  assert.equal(parseSprintlyPairingIntent({
    scheme: 'https', authority: 'sprintly', path: '/connect', query: 'code=A1B2C3D4E5F6',
  }), null);
  assert.equal(parseSprintlyPairingIntent({
    scheme: 'vscode', authority: 'sprintly', path: '/other', query: 'code=A1B2C3D4E5F6',
  }), null);
  assert.deepEqual(parseSprintlyPairingIntent({
    scheme: 'vscode', authority: 'sprintly', path: '/connect', query: 'code=A1B2C3D4E5F6&token=secret',
  }), { code: 'A1B2C3D4E5F6' });
});
