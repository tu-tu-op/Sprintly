...
function categorizeFailure(output, commandLine = '') {
    const matched = exports.FAILURE_PATTERNS.find(({ pattern }) => pattern.test(output));
    if (matched)
        return matched.id;
    const commandCategory = (0, terminalCommands_1.classifyTerminalCommand)(commandLine);
    if (commandCategory === 'build')
        return 'build_failure';
    if (commandCategory === 'test')
        return 'test_failure';
    if (commandCategory === 'lint')
        return 'lint_failure';
    return 'other';
}
